...
import { classifyCaixaTransaction } from "../classification/classify-caixa-transaction";
import type {
  CaixaAssignment,
  CaixaSignal,
} from "../classification/caixa-assignment-rules";

export type CaixaParsedTransaction = {
  accountId: string;
  bankName: string;
  companyName: string;
  date: string;
...
function findHeaderRowIndex(rows: SheetRow[]): number {
  return rows.findIndex((row) => {
    ...
    const hasLaunchDate = normalizedCells.some((cell) =>
      cell.includes("DATA LANC"),
    );
    const hasMoveDate = normalizedCells.some((cell) =>
      cell.includes("DATA MOV"),
    );
    const hasHistory = normalizedCells.includes("HISTORICO");
    const hasValue = normalizedCells.some((cell) => cell.includes("VALOR"));
    const hasBalance = normalizedCells.some((cell) => cell.includes("SALDO"));

    return hasLaunchDate && hasMoveDate && hasHistory && hasValue && hasBalance;
  });
}
...
function getColumnIndexes(headerRow: SheetRow) {
  ...
  const launchDateIndex = normalizedHeader.findIndex((cell) =>
    cell.includes("DATA LANC"),
  );
  const moveDateIndex = normalizedHeader.findIndex((cell) =>
    cell.includes("DATA MOV"),
  );
...
function hasRequiredColumns(
  ...
  return (
    columns.launchDateIndex !== -1 &&
    columns.moveDateIndex !== -1 &&
...
export function parseCaixaExtrato(
  input: ParseCaixaExtratoInput,
): CaixaParsedTransaction[] {
  ...
  if (headerRowIndex === -1) {
    throw new Error(
      "Cabeçalho do extrato da Caixa Econômica Federal não encontrado.",
    );
  }
  ...
  if (!hasRequiredColumns(columnIndexes)) {
    throw new Error(
      "As colunas obrigatórias do extrato da Caixa Econômica Federal não foram encontradas.",
    );
  }

  const transactions: CaixaParsedTransaction[] = [];
  ...
  for (let index = headerRowIndex + 1; index < rows.length; index++) {
    ...
    const moveDate =
      getCellDateValue(worksheet, index, columnIndexes.moveDateIndex) ||
      String(row[columnIndexes.moveDateIndex] ?? "").trim();
    const launchDate =
      getCellDateValue(worksheet, index, columnIndexes.launchDateIndex) ||
      String(row[columnIndexes.launchDateIndex] ?? "").trim();
    const description =
    ...
    const date = launchDate || moveDate;
    ...
    const { assignment, signal } = classifyCaixaTransaction({
      description,
    });
